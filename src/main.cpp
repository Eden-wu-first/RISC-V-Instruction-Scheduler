...
void runScheduler(const std::string& name, const std::vector<std::string>& code) {
    ...
    std::vector<DAGNode> nodes = dagBuilder.build(instructions);
    
    // Step 3: Schedule instructions
    Scheduler scheduler;
    ScheduleResult result = scheduler.schedule(instructions, nodes, TieBreakingPolicy::SMALLER_INDEX);

    // Step 4: Print scheduling result and comparison analysis
    scheduler.printSchedule(result, instructions, nodes);
    scheduler.printComparison(instructions, nodes, result);
}
