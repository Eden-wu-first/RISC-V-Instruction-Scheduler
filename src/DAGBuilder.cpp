...
std::vector<DAGNode> DAGBuilder::build(const std::vector<Instruction>& instructions) {
    ...
    // lastWriter: register -> last instruction index that wrote to it
    // readers: register -> list of instructions that read from it after the last write
    ...
    for (size_t i = 0; i < instructions.size(); i++) {
        ...
        if (inst.hasDestReg()) {
            ...
            if (it != readers.end()) {
                ...
                it->second.clear();  // Clear readers after adding dependencies
            }
        }

        // Detect WAW dependencies
        if (inst.hasDestReg()) {
            auto it = lastWriter.find(inst.dest_reg);
            if (it != lastWriter.end()) {
                int prevWriterIdx = it->second;
                addEdge(nodes, prevWriterIdx, static_cast<int>(i), DependencyType::WAW, inst.dest_reg);
            }
        }

        // Update last writer and readers
        if (inst.hasDestReg()) {
...
void DAGBuilder::detectWAR(const std::vector<Instruction>& instructions,
    ...
    for (size_t i = 0; i < instructions.size(); i++) {
        ...
        // Check if current instruction writes to a register
        if (inst.hasDestReg()) {
        ...
        // Update readers for source registers
        for (const auto& srcReg : inst.src_regs) {
